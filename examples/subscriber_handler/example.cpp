...
{
  RCLCPP_ERROR_STREAM(rclcpp::get_logger("timeout_callback"),
                      "Have not received a message on topic '" << topic_name << "' for " << (node->now() - last_msg).seconds() << " seconds");
}
...
public:
  ...
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("timer method"),
                        "Object has not received a message on topic '" << topic_name << "' for " << (node->now() - last_msg).seconds() << " seconds");
  }
} sub_obj;
...
int main(int argc, char** argv)
{
  ...
  mrs_lib::SubscriberHandler<std_msgs::msg::String> handler(shopts, topic_name, std::bind(timeout_callback, node, std::placeholders::_1, std::placeholders::_2),
                                                            message_callback);

  /* A variation of the factory method for easier use with objects also exists. */
  mrs_lib::construct_object(handler, shopts, topic_name, no_message_timeout,
