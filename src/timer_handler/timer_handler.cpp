...
namespace mrs_lib
{
  ...
  void ROSTimer::setPeriod(const double& duration)
  {
    ...
    if (timer_)
    {
      ...
      {
        this->timer_ = node_->create_timer(std::chrono::duration<double>(duration), callback, this->callback_group_.value());
      } else
      {
        this->timer_ = node_->create_timer(std::chrono::duration<double>(duration), callback);
      }
    }
  }
  ...
  void ThreadTimer::setPeriod(const double& duration)
  {
  ...
  ThreadTimer::Impl::Impl(const rclcpp::Node::SharedPtr& node, const std::function<void()>& callback, const double& rate, const bool oneshot)
      : oneshot_(oneshot)
  {
  ...
  void ThreadTimer::Impl::start()
  {
    ...
    if (!running_)
    {
      next_expected_ = node_->get_clock()->now() + rclcpp::Duration(std::chrono::duration<double>(delay_dur_));
      running_ = true;
      wakeup_cond_.notify_all();
    }
  }
  ...
  void ThreadTimer::Impl::setPeriod(const double& duration)
  {
