...
namespace mrs_lib
{
  ...
  LKF_MRS_odom::P_t LKF_MRS_odom::covariance_predict_optimized(const LKF_MRS_odom::P_t& P, const LKF_MRS_odom::Q_t& Q, double dt) const
  {
    ...
    P_ret(0, 5) = (P16 + P26*dt + P36*dtsqhalf)*(dt)*(p1) + Q(0, 5);
    ...
    P_ret(1, 5) = (P26 + P36*dt)*(dt)*(p1) + Q(1, 5);
    ...
    P_ret(3, 5) = P66*p3*(dt)*(p1) + Q(3, 5);
    P_ret(4, 0) = P51 + P52*(dt) + P53*(dtsqhalf) + Q(4, 0);
    P_ret(4, 1) = P52 + P53*(dt) + Q(4, 1);
    P_ret(4, 2) = P54 + P55 + Q(4, 2);
    P_ret(4, 3) = P56*(p3) + Q(4, 3);
    P_ret(4, 4) = P55 + Q(4, 4);
    P_ret(4, 5) = P56*(dt)*(p1) + Q(4, 5);
    P_ret(5, 0) = dt*p1*(P61 + P62*(dt) + P63*(dtsqhalf)) + Q(5, 0);
    P_ret(5, 1) = dt*p1*(P62 + P63*(dt)) + Q(5, 1);
    P_ret(5, 2) = dt*p1*(P64 + P65) + Q(5, 2);
    P_ret(5, 3) = P66*dt*p1*(p3) + Q(5, 3);
    P_ret(5, 4) = P65*dt*p1 + Q(5, 4);
    P_ret(5, 5) = P66*dt*p1*(dt)*(p1) + Q(5, 5);
    return P_ret;
  };
  //}
