...
BatchVisualizer::BatchVisualizer(ros::NodeHandle& nh, std::string marker_topic_name, std::string parent_frame) {
  ...
  this->visual_pub = std::make_shared<ros::Publisher>(nh.advertise<visualization_msgs::MarkerArray>(marker_topic_name.c_str(), 1));

  publish();
}

BatchVisualizer::BatchVisualizer(const BatchVisualizer& other) {

  this->parent_frame      = other.parent_frame;
  this->marker_topic_name = other.marker_topic_name;

  this->points_marker    = other.points_marker;
  this->lines_marker     = other.lines_marker;
  this->triangles_marker = other.triangles_marker;

  this->visual_pub = other.visual_pub;
}

BatchVisualizer& BatchVisualizer::operator=(const BatchVisualizer& other) {
...
void BatchVisualizer::setParentFrame(const std::string parent_frame) {
  ...
  {
    std::scoped_lock lock(mutex_points_marker_, mutex_lines_marker_, mutex_triangles_marker_);
...
void BatchVisualizer::addPoint(Eigen::Vector3d p, double r, double g, double b, double a) {
  ...
  {
    std::scoped_lock lock(mutex_points_marker_);
...
void BatchVisualizer::addRay(mrs_lib::geometry::Ray ray, double r, double g, double b, double a) {
  ...
  {
    std::scoped_lock lock(mutex_lines_marker_);
...
void BatchVisualizer::addTriangle(mrs_lib::geometry::Triangle tri, double r, double g, double b, double a, bool filled) {
  ...
  if (filled) {
    ...
    {
      std::scoped_lock lock(mutex_triangles_marker_);
...
void BatchVisualizer::clearVisuals() {
  ...
  visualization_msgs::Marker points_tmp;
  visualization_msgs::Marker lines_tmp;
  visualization_msgs::Marker triangles_tmp;

  {
    std::scoped_lock lock(mutex_points_marker_, mutex_lines_marker_, mutex_triangles_marker_);

    points_tmp    = points_marker;
    lines_tmp     = lines_marker;
    triangles_tmp = triangles_marker;
  }
...
void BatchVisualizer::publish() {
  ...
  bool resolve_empty_points;
  bool resolve_empty_lines;
  bool resolve_empty_triangles;

  {
    std::scoped_lock lock(mutex_points_marker_, mutex_lines_marker_, mutex_triangles_marker_);

    resolve_empty_points    = points_marker.points.empty();
    resolve_empty_lines     = lines_marker.points.empty();
    resolve_empty_triangles = triangles_marker.points.empty();
  }
  ...
  try {
    ...
    ROS_ERROR("exception caught during publishing topic '%s'", visual_pub->getTopic().c_str());
  }
