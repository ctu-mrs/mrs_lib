...
namespace mrs_lib::MRS_LIB_INTERNAL_INLINE_API_V1 v1
{
  ...
  /* constructor //{ */
  DynparamMgr::DynparamMgr(const std::shared_ptr<rclcpp::Node>& node, std::mutex& mtx) : m_node(node), m_pp(node), m_load_successful(true), m_mtx(mtx)
  {
  ...
  rcl_interfaces::msg::SetParametersResult DynparamMgr::cbk_param_update(const std::vector<rclcpp::Parameter>& parameters)
  {
    ...
    for (const auto& new_param : parameters)
    {
      ...
      const auto found_it = std::find_if(std::begin(m_registered_params), std::end(m_registered_params),
                                         [&param_name](const auto& registered_param) { return registered_param.resolved_name.str == param_name; });
      ...
      case type_enum::PARAMETER_BOOL:
        result.successful = found_el.update_value(new_param.as_bool());
        break;
      case type_enum::PARAMETER_INTEGER:
        result.successful = found_el.update_value(new_param.as_int());
        break;
      case type_enum::PARAMETER_DOUBLE:
        result.successful = found_el.update_value(new_param.as_double());
        break;
      case type_enum::PARAMETER_STRING:
        result.successful = found_el.update_value(new_param.as_string());
        break;
      case type_enum::PARAMETER_BOOL_ARRAY:
        result.successful = found_el.update_value(new_param.as_bool_array());
        break;
      case type_enum::PARAMETER_BYTE_ARRAY:
        result.successful = found_el.update_value(new_param.as_byte_array());
        break;
      case type_enum::PARAMETER_INTEGER_ARRAY:
        result.successful = found_el.update_value(new_param.as_integer_array());
        break;
      case type_enum::PARAMETER_DOUBLE_ARRAY:
        result.successful = found_el.update_value(new_param.as_double_array());
        break;
      case type_enum::PARAMETER_STRING_ARRAY:
        result.successful = found_el.update_value(new_param.as_string_array());
        break;
  ...
  rclcpp::ParameterValue DynparamMgr::registered_param_t::to_param_val() const
  {
    try
    {
      return std::visit([](auto arg) { return rclcpp::ParameterValue(*arg); }, param_ptr);
    }
