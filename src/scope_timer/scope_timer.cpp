...
std::chrono::milliseconds::rep ScopeTimer::getLifetime() {
  const auto lifetime = (std::chrono::steady_clock::now() - checkpoints.at(0).chrono_time);
  return std::chrono::duration_cast<std::chrono::milliseconds>(lifetime).count();
}
