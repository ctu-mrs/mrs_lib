...
namespace mrs_lib {
    ...
    bool Polygon::isClockwise() {
       ...
       for (int i = 0; i < vertices.rows(); ++i) {
           ...
           if ((edge(0, 1) <= center(1) && edge(1, 1) <= center(1)) ||
               (edge(0, 1) > center(1) && edge(1, 1) > center(1)))
               continue;
           double intersect_x = edge(0, 0) +
                   (edge(1, 0) - edge(0, 0)) / (edge(1, 1) - edge(0, 1)) * (center(1) - edge(0, 1));
    ...
    static Intersection lineIntersectGivenVector(Eigen::RowVector2d start1, Eigen::RowVector2d vector1,
       Eigen::RowVector2d start2, Eigen::RowVector2d vector2) 
    {
