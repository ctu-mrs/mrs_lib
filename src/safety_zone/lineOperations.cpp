...
namespace mrs_lib {
  Intersection::Intersection(bool intersect, bool parallel, Eigen::RowVector2d point) :
     point(std::move(point)), parallel(parallel), intersect(intersect) {};
     ...
     Eigen::RowVector2d start2, Eigen::RowVector2d end2) 
  {
    ...
    if (cross == 0) {
      ...
      if (0 <= start2Scale && start2Scale <= 1) return Intersection{true, true, start2};
      double end2Scale = getScale(start1, vector1, end2);
      if (0 <= end2Scale && end2Scale <= 1) return Intersection{true, true, end2};
