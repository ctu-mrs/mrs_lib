...
/*   std::cout << param << std::endl; */
  ...
  pl.load_param_dynamic("PID.kP", kP, 0.01, descr);
  pl.load_param_reusable("PID.kP", kP);
  pl.load_param("some_string", some_string, "this is the default value of the some_string parameter");
  pl.load_param("PID.kI", kI);
  const auto uav_name = pl.load_param2<std::string>("uav_name");
  const auto uav_type = pl.load_param2<std::string>(std::string("uav_type"));
