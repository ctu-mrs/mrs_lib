...
namespace mrs_lib
{
  ...
  public:
     ...
     *
     */
    using valid_types_t = std::tuple<int, int64_t, float, double, bool, std::string, std::vector<uint8_t>, std::vector<int64_t>, std::vector<double>,
     ...
     *
     * This method registers the provided pointer to a variable and a name of a ROS parameter to keep the pointed-to variable up-to-date with the value of the
     * ROS parameter.
     *
     * It will:
     ...
     * 2. Load its default value using ParamProvider - this means the default value can be loaded from ROS or from a provided YAML file, if available, or return
     * false if the loading fails.
     ...
     * \param param_var       Pointer to the variable whose value will be changed when the parameter is updated. It is the user's responsibility to ensure that
     * the lifetime of the pointed-to variable is valid while it is being used by the DynparamMgr. \param update_cbk      Optional function to be called when
     * the parameter is changed. \return                true iff the parameter was successfully declared and its default value loaded.
     *
     * \note The C++ type passed to this function will be mapped to one of the valid values of rclcpp::ParameterType. Not all C++ types are supported (see
     * valid_types_t).
     */
     ...
     * \param name            Name of the parameter (this will be used for the ROS declaration etc.).
     * \param param_var       Pointer to the variable whose value will be changed when the parameter is updated. It is the user's responsibility to ensure that
     * the lifetime of the pointed-to variable is valid while it is being used by the DynparamMgr. \param default_value   Optional default value to be set if no
     * value could be loaded from ParamProvider. \param update_cbk      Optional function to be called when the parameter is changed. \return true iff the
     * parameter was successfully declared and its default value loaded.
     *
     * \note The C++ type passed to this function will be mapped to one of the valid values of rclcpp::ParameterType. Not all C++ types are supported (see
     * valid_types_t).
     */
     ...
     * \param param_var       Pointer to a variable whose value will be changed when the parameter is updated. It is the user's responsibility to ensure that
     * the lifetime of the pointed-to variable is valid while it is being used by the DynparamMgr. \param valid_range     Range of valid values (inclusive) of
     * the parameter. \param update_cbk      Optional function to be called when the parameter is changed. \return                true iff the parameter was
     * successfully declared and its default value loaded.
     */
    template <typename MemT>
     ...
     * \param name            Name of the parameter (this will be used for the declaration etc.).
     * \param param_var       Pointer to a variable whose value will be changed when the parameter is updated. It is the user's responsibility to ensure that
     * the lifetime of the pointed-to variable is valid while it is being used by the DynparamMgr. \param default_value   Default value to be set if no value
     * could be loaded from ParamProvider. \param valid_range     Range of valid values (inclusive) of the parameter. \param update_cbk      Optional function
     * to be called when the parameter is changed. \return                true iff the parameter was successfully declared and its default value loaded.
     */
    template <typename MemT>
    bool register_param(const std::string& name, MemT* param_var, const MemT& default_value, const range_t<MemT>& valid_range,
  ...
  private:
    ...
    template <typename MemT>
    bool register_param_impl(const std::string& name, MemT* param_var, const std::optional<MemT>& default_value,
