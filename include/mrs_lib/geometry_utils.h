...
namespace mrs_lib
{
  namespace geometry
  {
    ...
    template <typename flt, flt minimum, flt supremum>
    flt cycInRange(const flt a)
    {
      return a >= minimum && a < supremum;
    }

    template <typename flt, flt minimum, flt supremum>
    flt cycWrap(const flt a)
    {
      constexpr double range = supremum - minimum;
      const double remainder = std::fmod(a - minimum, range);
      const double wrapped = remainder + minimum + 
      if (ret < 0.0)
        ret += range;
      return ret + minimum;
    }
    ...
    template <typename flt, flt minimum, flt supremum>
    flt cycPDist(const flt a, const flt b);
    template <typename flt, flt minimum, flt supremum>
    flt cycSDist(const flt a, const flt b);
